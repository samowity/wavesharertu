...
def _build_parser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(
		...
		description="Read and configure Waveshare Modbus RTU Analog Input 8CH parameters.",
	)
	...
	try:
		with RS485(port=args.port, baudrate=args.baudrate, parity=args.parity) as serial_port:
			...
			try:
				...
				logging.error(
					"Failed to communicate with device. Check connection and parameters. Error: %s", exc)
				return 1
			
			change_modes = prompt_user_choice(
			...
			if change_modes == "y":
				...
				change_individual_modes = prompt_user_choice(
